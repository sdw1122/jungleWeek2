...
if(!plants.length){empty.hidden=false;}else{
  plants.forEach((plant,index)=>{
    ...
    card.innerHTML=`<div class="plant-photo" style="background-image:url('${plant.image||''}')"></div><div class="plant-body"><div class="plant-top"><div><small>PLANT ${String(index+1).padStart(2,'0')}</small><h2>${(plant.name||'식물').replace(prefixPattern,'')}</h2></div><span class="stage-badge">${stage} 단계</span></div><div class="plant-energy"><span>성장 에너지</span><b>${energy} / 100</b></div><div class="energy-track"><i style="width:${energy}%"></i></div><div class="plant-enter"><span>상태창 들어가기</span><b>→</b></div></div>`;
